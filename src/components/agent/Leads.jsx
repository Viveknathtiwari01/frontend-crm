...
const Leads = () => {
  ...
  useEffect(() => {
    const fetchLeads = async () => {
      try {
        ...
        // Fetch leads specific to the user
        const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/leads/userId/${userId}`);
  ...
  const handleSubmitLead = async () => {
    try {
      // Retrieve the current user's name or ID from localStorage
      const token = localStorage.getItem("authToken"); // Or however you store the token
      const decodedToken = JSON.parse(atob(token.split(".")[1])); // Decode JWT payload
      const userId = decodedToken.id;
      if (!userId) {
      ...
      // Add the agent ID or name to the new lead data
      const leadData = {
        ...newLead,
        userId, // Assuming the backend handles assignment by user ID
      };
  
      // Submit the new lead to the server
      const response = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/leads/addAgentLead`, leadData);
  
      // Update the leads state with the newly created lead
      setLeads((prev) => (Array.isArray(prev) ? [...prev, response.data] : [response.data]));
  
      toast.success("Lead created successfully!");
  
      // Fetch the updated list of leads specific to the logged-in agent
      const updatedLeads = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/leads/userId/${userId}`);
